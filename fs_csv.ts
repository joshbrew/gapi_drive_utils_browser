import { ZenFsRoutes, appendFile, deleteFile, exists, getCSVHeader, listFiles, processCSVChunksFromDB, readCSVChunkFromDB, writeFile, writeToCSVFromDB } from './zenfsUtils'; //'./BFSUtils'
...
export async function checkFolderList(listfilename = 'data/folderList', hasFolderName?) {
    if (hasFolderName && !(await ZenFsRoutes.exists(listfilename))) {
        ...
        await ZenFsRoutes.deleteFile(listfilename);
        await createCSV(listfilename, ['folder', 'lastmodified']);
        await appendCSV(data, listfilename);
...
export async function parseFolderList(listParentFolder: string = 'data'): Promise<string[]> {
